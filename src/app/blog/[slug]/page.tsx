    ...
    return (
        <div className='my-16 md:my-28 overflow-x-hidden'>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-x-6 lg:gap-x-6 p-6 md:p-0">
                ...
                <div className={`md:col-start-4 md:col-span-6 ${negativeMarginTitleAndHero} ${paddingClasses} flex flex-col gap-3 relative`}>

                    {/* Pattern */}
                    <div className="absolute z-0 w-full bg-pattern top-0 h-full scale-x-125">
                        {/* Nothing goes here */}
                    </div>

                    {/* Draft Indicator */}
                    {
                        postResGraphQL?.status === 'draft' &&
                        <div className="inline-block z-10 self-start px-3 py-1 rounded-full bg-slate-200 dark:bg-slate-700">
                            <div className="flex items-center justify-center h-full">
                                <span className="text-slate-800 dark:text-slate-100">Draft</span>
                            </div>
                        </div>
                    }
                    <h1 className={`text-3xl md:text-4xl xl:text-5xl font-bold z-10 relative text-slate-800 dark:text-slate-100`}>
                        {/* Post Title */}
                        {postResGraphQL && postResGraphQL.title}
                    </h1>
                    {/* Post Date */}
                    <div className=" inline-flex flex-wrap items-baseline z-10 relative text-slate-700 dark:text-slate-300">
...
const RecentPosts = ({ recentPosts }: { recentPosts: Array<any> }) => {
    return (
        <div className="recent-posts">
            <h2 className='text-sky-700 dark:text-sky-600 text-3xl font-light leading-4 mb-6'>Recent Posts</h2>
            <ul>
                {recentPosts?.map?.(({ node: post }, index: number) =>

                    <li key={`recent-post-${index}`}>
                        <Link href={`/blog/${post.slug || post.databaseId}`} className=' text-gray-600 hover:text-gray-800 dark:text-slate-400 dark:hover:text-slate-300 font-bold block mb-2 '>{post.title}</Link>
                    </li>
                )}
            </ul>
        </div>
