...
const tilliumWeb = localFont({
    display: 'swap',
    src: [
        {
            path: './../fonts/Titillium_Web/TitilliumWeb-Light.ttf',
            weight: '300',
            style: 'normal'
        },
        {
...
export default async function RootLayout({
    ...
    return (
        <html lang="en" className={`h-full ${theme}`}>
            <body className={`${tilliumWeb.className} ${bgClassNames} h-full flex flex-col text-slate-800 dark:text-slate-200 ${bgClassNames} overflow-x-hidden z-0`}>

                <Navigation></Navigation>
                <div id="content-wrapper" className={`${bgClassNames} flex flex-col flex-grow border-b border-b-slate-300 dark:border-b-slate-700 `}>
                    <GATracker>
                        <AnimatedDiv>
                            {children}
                        </AnimatedDiv>
