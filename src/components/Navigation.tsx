...
function Navigation({ path }: Props) {
    ...
    const [blogTitlePreviewInNav, setBlogTitlePreviewInNav] = useState('');
    ...
    useEffect(() => {
        ...
        // if (pathName.includes('/blog/')) {
            ...
            setBlogTitlePreviewInNav(blogTitle);
            if (blogTitleEl) {
    ...
    return (
        <nav className='fixed flex flex-col sm:flex-row justify-between items-center p-4 top-0 left-0 right-0 text-sky-700 w-full z-10
            bg-gradient-to-b from-white dark:from-slate-900 to-transparent'
        >
            {blogTitlePreviewInNav &&
                <BlogTitlePreview blogTitlePreviewInNav={blogTitlePreviewInNav} titleIsInView={titleIsInView}></BlogTitlePreview>
            }

            <div className='z-30 xl:container flex order-1 sm:order-2 py-4 md:py-6 px-6
                lg:px-12 gap-8 items-center ml-auto '
            >

                <ToggleTheme svgSize={svgSize}></ToggleTheme>
                <Link href='/blog'>Blog</Link>
                <Link href='/'>Home</Link>
                <LinkExt href='mailto:paulmerupu91@gmail.com'
                ...
                <LinkExt href='https://www.linkedin.com/in/paulmerupu'
            ...
            <animated.div style={{...stylesFromSpring}} className=' border border-slate-200 dark:border-slate-700 bg-slate-200 dark:bg-slate-950 bg-opacity-50 z-20 absolute m-4 top-0 bottom-0 left-0 right-0 rounded-full backdrop-blur-lg '></animated.div>
...
function BlogTitlePreview({ blogTitlePreviewInNav, titleIsInView }: { blogTitlePreviewInNav: string, titleIsInView: boolean }) {
    ...
    return (
        ...
        <animated.div style={props} className='order-2 z-30 hidden sm:inline-block sm:order-1 text-sm px-4 mx-6 py-2 rounded-full bg-slate-200 dark:bg-slate-900'>
            Reading: {blogTitlePreviewInNav}
        </animated.div>
    )
